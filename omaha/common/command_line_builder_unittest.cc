...
// The /update builder works when not used with ElementUpdate.exe.
...
// The /update builder should not be used with ElementUpdate.exe directly.
TEST(CommandLineBuilder, BuildUpdateAndGetCommandLineWithGoogleUpdateExe) {
  CommandLineBuilder builder(COMMANDLINE_MODE_UPDATE);
  ExpectAsserts expect_asserts;
  CString cmd_line = builder.GetCommandLine(_T("C:\\ElementUpdate.exe"));
  EXPECT_STREQ(_T("\"C:\\ElementUpdate.exe\" /update"), cmd_line);
}
